...
<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1rem">
    <div>
        ...
        <h1 style="margin:.25rem 0 0;font-size:1.4rem;font-weight:700"><?= e($name) ?></h1>
...
<form method="POST" action="/admin/system-layouts/<?= e(rawurlencode($name)) ?>">
    ...
    <!-- Placement editor — reuses the page-layout admin's row partial -->
    <div class="card" style="margin-bottom:1rem">
        ...
        <div class="card-body">
            ...
            <table class="table" id="placement-table" style="font-size:13px">
                ...
                <?php foreach ($placements as $i => $p): ?>
                    <?php include BASE_PATH . '/modules/pages/Views/admin/_layout_row.php'; ?>
                <?php endforeach; ?>
...
<!-- Hidden template for new placement rows. Same partial the page-layout
     editor uses — the form field names + structure match. -->
<template id="placement-template">
    <?php
        $i = '__INDEX__';
        $p = ['row_index' => 0, 'col_index' => 0, 'sort_order' => 0, 'block_key' => '', 'settings' => [], 'visible_to' => 'any'];
        include BASE_PATH . '/modules/pages/Views/admin/_layout_row.php';
    ?>
</template>
...
/* Per-row block label + delete-mark visualisation. Same handlers the
   per-page composer editor uses; the row partial (modules/pages/Views/admin/
   _layout_row.php) ships with the matching markup. */
