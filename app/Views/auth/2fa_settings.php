...
<!-- Status banner -->
<div class="card" style="margin-bottom:1.25rem;padding:1.5rem;display:flex;align-items:center;gap:1rem;border-left:4px solid <?= $enabled ? 'var(--color-success)' : 'var(--color-gray-200)' ?>">
    <div style="font-size:2rem;flex-shrink:0"><?= $enabled ? '🛡️' : '🔓' ?></div>
    <div style="flex:1">
        <div style="font-weight:700;font-size:1rem;color:<?= $enabled ? 'var(--color-success-fg)' : 'var(--color-gray-700)' ?>">
            Two-factor authentication is <?= $enabled ? 'enabled' : 'disabled' ?>
        </div>
        <?php if ($enabled): ?>
        <div style="color:var(--color-gray-500);font-size:13.5px;margin-top:.2rem">
            Active method:
            <?php $labels = ['email'=>'Email OTP','sms'=>'SMS OTP','totp'=>'Authenticator App (TOTP)']; ?>
            <strong><?= e($labels[$method] ?? ucfirst($method ?? '')) ?></strong>
        </div>
        <?php else: ?>
        <div style="color:var(--color-gray-500);font-size:13.5px;margin-top:.2rem">Add an extra layer of security to your account.</div>
        <?php endif; ?>
    </div>
    <?php if ($enabled): ?>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
            ...
            <form method="POST" action="/profile/2fa/enable">
                ...
                <button type="submit" style="width:100%;background:#fff;border:2px solid var(--color-gray-200);border-radius:10px;padding:1.25rem;text-align:left;cursor:pointer;font-family:inherit;transition:border-color .15s" onmouseover="this.style.borderColor='var(--color-success)'" onmouseout="this.style.borderColor='var(--color-gray-200)'">
                    ...
                    <div style="font-size:11px;color:var(--color-warning);margin-top:.35rem">⚠️ Requires a phone number on your profile.</div>
                    <?php endif; ?>
                </button>
            </form>
