...
//
// Creates the framework's baseline schema and seed data — replaces the
// legacy database/install.sql one-shot script.
//
// Why a baseline migration: install.sql required users to run
// `mysql < install.sql` BEFORE `php artisan migrate`. That broke fresh
// installs (the README's Quick start was incomplete) and made any flow
// that provisions databases programmatically (e.g. multi-tenant SaaS
// like claudephpwebappbuilder spinning up a tenant DB on signup) shell
// out to mysql for every new tenant. Folding the baseline into a
// migration means: drop a fresh DB, run `php artisan migrate`, done.
//
// Content traced verbatim from the 2026-04-19 install.sql (which itself
// merged schema.sql, 2fa_migration.sql, security_fixes_migration.sql,
// performance_indexes_migration.sql).
//
// Two earlier supplementary scripts that LIVED in database/ but were
// never folded into install.sql — owner_removal_outcome_migration.sql
// and webhook_channel_migration.sql — were verified in 2026-05-02 to
// already be reflected in install.sql's CREATE TABLE statements (the
// new_role_id column on group_owner_removal_requests, and the 'webhook'
// member of the message_log.channel ENUM). So this migration is the
// complete baseline; no follow-up backfill is required.
//
// unescape_user_data_migration.sql was a one-time data cleanup after a
// double-escape Validator bug. Its underlying bug is fixed; new installs
// have no encoded backlog to undo. Not included.
...
SQL;
     ...
     * only statements.
     *
     * Limitations: doesn't try to handle ';' inside string literals.
     * None of the baseline INSERTs contain ';' inside quoted values, so
     * naive splitting is safe here. If a future bootstrap migration
     * needs ';' inside strings, write its statements one at a time
