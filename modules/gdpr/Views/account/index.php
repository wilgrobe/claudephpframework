...
<div class="card" style="margin-bottom:1.25rem">
    <div class="card-body" style="padding:1.25rem">
        ...
        <?php if (!empty($exports)): ?>
            <div style="margin-top:1rem;border-top:1px solid var(--color-gray-100);padding-top:.75rem">
                <div style="font-size:12px;color:var(--color-gray-500);margin-bottom:.5rem;text-transform:uppercase;letter-spacing:.04em">Recent exports</div>
                <?php foreach ($exports as $exp): ?>
                    <div style="display:flex;justify-content:space-between;align-items:center;padding:.4rem 0;font-size:13px;border-bottom:1px dashed var(--color-gray-100)">
                        <div>
                        ...
                        <div>
                            ...
                            <?php elseif ($exp['status'] === 'expired'): ?>
                                <span style="color:var(--color-gray-400);font-size:12px">expired</span>
                            <?php elseif ($exp['status'] === 'failed'): ?>
                                <span style="color:var(--color-danger);font-size:12px">failed</span>
                            <?php else: ?>
                                <span style="color:var(--color-gray-500);font-size:12px"><?= htmlspecialchars((string) $exp['status'], ENT_QUOTES) ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
...
<div class="card" style="margin-bottom:1.25rem">
    <div class="card-body" style="padding:1.25rem">
        <h2 style="margin:0 0 .5rem;font-size:1.05rem">
            Restrict processing
            <?php if ($restricted): ?>
                <span style="font-size:11px;background:var(--color-warning-bg);color:var(--color-warning-fg);padding:.15rem .5rem;border-radius:999px;margin-left:.5rem">RESTRICTED</span>
            <?php endif; ?>
        </h2>
        <p style="margin:0 0 1rem;color:var(--color-gray-500);font-size:13px;line-height:1.6">
...
<div class="card" style="margin-bottom:1.25rem;border-color:#fecaca">
    <div class="card-body" style="padding:1.25rem">
        ...
        <form method="POST" action="/account/data/erase"
            ...
            <?= csrf_field() ?>
            <label for="confirm" style="display:block;font-size:12.5px;color:var(--color-gray-500);margin-bottom:.4rem">
...
<div class="card" style="margin-bottom:1.25rem">
    <div class="card-body" style="padding:1.25rem">
        ...
        <p style="margin:0 0 1rem;color:var(--color-gray-500);font-size:12.5px">A record of every formal request you've filed.</p>
        <?php foreach ($dsar as $d): ?>
            <div style="display:flex;justify-content:space-between;padding:.4rem 0;border-bottom:1px dashed var(--color-gray-100);font-size:13px">
                <div>
                    <strong><?= htmlspecialchars(ucfirst((string) $d['kind']), ENT_QUOTES) ?></strong>
                    <span style="color:var(--color-gray-500)">requested <?= htmlspecialchars(date('M j, Y', strtotime((string) $d['requested_at'])), ENT_QUOTES) ?></span>
                </div>
