...
<div class="card" style="margin-bottom:1rem">
    <div class="card-header" style="padding:.75rem 1rem;display:flex;justify-content:space-between;align-items:center">
        <div>
            ...
            <div style="font-size:12px;color:var(--color-gray-500);margin-top:.15rem"><?= htmlspecialchars((string) $h->description, ENT_QUOTES) ?></div>
        </div>
    ...
    <table class="table" style="width:100%;font-size:12.5px;margin:0">
        ...
        <tbody>
            <?php foreach ($h->tables as $t):
                ...
                $colors = ['erase' => 'var(--color-danger)', 'anonymize' => 'var(--color-warning)', 'keep' => 'var(--color-success)'];
                $color  = $colors[$action] ?? 'var(--color-gray-500)';
            ...
            ?>
                <tr style="border-top:1px solid var(--color-gray-100)">
                    ...
                    <td style="padding:.4rem .75rem;color:var(--color-gray-500)">
                        ...
                        <?= $skip ? '<span style="color:var(--color-gray-400)">· not exported</span>' : '' ?>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
