...
<div style="font-size:12px;color:var(--color-gray-500);margin-bottom:.25rem">
    <a href="/admin/retention" style="color:var(--color-primary);text-decoration:none">← Retention rules</a>
</div>
    ...
    <div class="card-body" style="padding:1rem 1.25rem">
        ...
        <form method="POST" action="/admin/retention/<?= (int) $rule['id'] ?>/edit">
            ...
            <div style="display:grid;grid-template-columns:auto auto 1fr;gap:.75rem;align-items:end">
                <label>
                    <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">Days kept</span>
                    <input type="number" name="days_keep" value="<?= (int) $rule['days_keep'] ?>" min="0" style="width:100px">
                </label>
                <label>
                    <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">Action</span>
                    <select name="action">
            ...
            <div style="margin-top:.75rem;font-size:12.5px;color:var(--color-gray-500);line-height:1.55">
...
<div class="card">
    ...
    <table class="table" style="width:100%;font-size:12.5px;margin:0">
        ...
        <tbody>
            <?php if (empty($runs)): ?>
                <tr><td colspan="6" style="padding:1.5rem;text-align:center;color:var(--color-gray-500)">No runs yet. Use the preview or run-now buttons above to see what this rule would do.</td></tr>
            <?php else: foreach ($runs as $r): ?>
                <tr style="border-top:1px solid var(--color-gray-100)">
                    ...
                    <td style="padding:.4rem .75rem;color:var(--color-gray-500)"><?= $r['triggered_by_username'] ? htmlspecialchars((string) $r['triggered_by_username'], ENT_QUOTES) : 'cron' ?></td>
