...
<div class="card" style="margin-bottom:1rem">
    <div class="card-body" style="display:flex;gap:.75rem;flex-wrap:wrap;padding:1rem">
        <?php
        $cards = [
            ['Total runs',    (int) ($stats['total_runs']      ?? 0), 'var(--color-gray-700)'],
            ['Total breaks',  (int) ($stats['total_breaks']    ?? 0), ($stats['total_breaks'] > 0 ? 'var(--color-danger)' : 'var(--color-success)')],
            ['Unack breaks',  (int) ($stats['unack_breaks']    ?? 0), ($stats['unack_breaks'] > 0 ? 'var(--color-danger)' : 'var(--color-success)')],
            ['Last run breaks',(int) ($stats['last_run_breaks']?? 0), ($stats['last_run_breaks'] > 0 ? 'var(--color-danger)' : 'var(--color-success)')],
        ];
    ...
    <div style="padding:.5rem 1rem;font-size:12.5px;color:var(--color-gray-500);border-top:1px solid var(--color-gray-100)">
        Last run: <?= $stats['last_run_at'] ? e(date('M j, Y g:ia T', strtotime((string) $stats['last_run_at']))) : '<em>never</em>' ?>
        · Last break:
        <?= $stats['last_break_at']
            ? '<strong style="color:var(--color-danger)">' . e(date('M j, Y g:ia T', strtotime((string) $stats['last_break_at']))) . '</strong>'
            : '<em>none</em>' ?>
    </div>
</div>
...
<div class="card" style="margin-bottom:1rem">
    <div class="card-body" style="padding:1rem 1.25rem">
        ...
        <form method="POST" action="/admin/audit-chain/verify" style="display:flex;gap:.5rem;align-items:end;flex-wrap:wrap">
            <?= csrf_field() ?>
            <label>
                <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">From</span>
                <input type="date" name="day_from" value="<?= e(date('Y-m-d', time() - 30 * 86400)) ?>">
            </label>
            <label>
                <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">To</span>
        ...
        <tbody>
            ...
            <?php else: foreach ($recent as $r): ?>
                <tr style="border-top:1px solid var(--color-gray-100);<?= ((int) ($r['breaks_found'] ?? 0)) > 0 ? 'background:var(--color-danger-bg)' : '' ?>">
                    ...
                    <td style="padding:.4rem .75rem;text-align:right;<?= ((int) ($r['breaks_found'] ?? 0)) > 0 ? 'color:var(--color-danger);font-weight:600' : 'color:var(--color-gray-700)' ?>"><?= $r['breaks_found'] !== null ? number_format((int) $r['breaks_found']) : '—' ?></td>
                    <td style="padding:.4rem .75rem;text-align:right;color:var(--color-gray-500)"><?= $r['duration_ms'] !== null ? (int) $r['duration_ms'] . ' ms' : '—' ?></td>
                    <td style="padding:.4rem .75rem;color:var(--color-gray-500)"><?= e($r['triggered_by_username'] ?? 'cron') ?></td>
                </tr>
