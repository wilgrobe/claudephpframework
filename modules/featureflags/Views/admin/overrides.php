...
<div class="card">
    ...
    <div class="card-body" style="color:var(--color-gray-400);text-align:center;padding:2rem 1rem">No per-user overrides.</div>
    <?php else: ?>
    <table class="table">
        ...
        <tr>
            <td>@<?= e((string) ($o['username'] ?? '?')) ?> <span style="color:var(--color-gray-400)">#<?= (int) $o['user_id'] ?></span></td>
            <td><?= (int) $o['enabled'] === 1 ? '✓ on' : '× off' ?></td>
            <td style="font-size:13px;color:var(--color-gray-500)"><?= e((string) ($o['note'] ?? '')) ?></td>
