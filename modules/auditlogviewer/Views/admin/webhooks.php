...
.wh-h1 .ct { color:#9ca3af; font-size:13px; font-weight:500; }
.wh-help   { color:#6b7280; font-size:13px; max-width:720px; line-height:1.5; margin:0 0 1rem; }

.wh-tabs   { display:flex; gap:.4rem; margin-bottom:.85rem; }
.wh-tab    { padding:.4rem .85rem; border-radius:6px; background:#fff; color:#374151; font-size:13px; font-weight:600; text-decoration:none; border:1px solid #e5e7eb; }
.wh-tab.active { background:#4f46e5; color:#fff; border-color:#4f46e5; }
...
.wh-filters { display:grid; gap:.5rem; grid-template-columns: 1fr 160px 160px 140px; margin-bottom:.85rem; background:#fff; border:1px solid #e5e7eb; border-radius:8px; padding:.75rem 1rem; }
.wh-filters input, .wh-filters button, .wh-filters .btn-secondary { padding:.45rem .65rem; border:1px solid #d1d5db; border-radius:4px; font-size:13px; font-family:inherit; }
.wh-filters .actions { grid-column: span 4; display:flex; gap:.4rem; justify-content:flex-end; }

.wh-table { width:100%; background:#fff; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; border-collapse:collapse; }
.wh-table th, .wh-table td { padding:.55rem .8rem; text-align:left; font-size:12.5px; border-bottom:1px solid #f3f4f6; }
.wh-table th { background:#fafafa; font-size:10.5px; text-transform:uppercase; letter-spacing:.3px; color:#6b7280; font-weight:700; }
...
.prov-ses        { background:#fef3c7; color:#92400e; }
.prov-sendgrid   { background:#dbeafe; color:#1e40af; }
...
.wh-sidebar h3 { font-size:11.5px; text-transform:uppercase; letter-spacing:.3px; color:#6b7280; margin:0 0 .5rem; font-weight:700; }
.wh-sidebar .card { padding:.75rem 1rem; }
.wh-chip-list { display:flex; flex-direction:column; gap:.2rem; }
.wh-chip { display:flex; justify-content:space-between; align-items:center; padding:.3rem .55rem; border-radius:4px; background:#fff; border:1px solid #e5e7eb; font-size:12px; text-decoration:none; color:#374151; }
.wh-chip:hover { background:#f9fafb; border-color:#c4b5fd; }
.wh-chip .ct { color:#9ca3af; font-size:10.5px; font-weight:600; }
.wh-chip.active { background:#ede9fe; border-color:#c4b5fd; color:#5b21b6; }

.wh-empty { padding:3rem 1rem; color:#9ca3af; text-align:center; font-style:italic; }
.wh-pager { display:flex; justify-content:space-between; align-items:center; margin-top:.85rem; font-size:12.5px; color:#6b7280; }
.wh-pager .nav a, .wh-pager .nav span.disabled { padding:.35rem .7rem; border:1px solid #d1d5db; border-radius:4px; background:#fff; color:#374151; text-decoration:none; margin-left:.25rem; }
.wh-pager .nav span.disabled { color:#d1d5db; }
</style>
...
<div class="wh-shell">
    <div class="wh-head">
        ...
        <div style="display:flex;gap:.5rem;align-items:center">
            <a href="/admin/activity" style="font-size:12.5px;color:#4f46e5;text-decoration:none">Activity →</a>
            <span style="color:#d1d5db">·</span>
            <a href="/admin/audit-log" style="font-size:12.5px;color:#4f46e5;text-decoration:none">Audit log →</a>
        </div>
    </div>
    ...
    <form method="get" class="wh-filters">
        ...
        <div class="actions">
            <button class="btn btn-primary" style="background:#4f46e5;color:#fff;border:1px solid #4f46e5;padding:.45rem .85rem;border-radius:4px;font-size:13px;cursor:pointer">Filter</button>
            <a href="/admin/webhooks" class="btn-secondary" style="padding:.45rem .85rem;text-decoration:none;color:#374151;border:1px solid #d1d5db;border-radius:4px;font-size:13px">Clear</a>
        </div>
    </form>
    ...
    <div class="wh-grid">
        <div>
            ...
            <?php else: ?>
                <table class="wh-table">
                    ...
                    ?>
                        <tr>
                            <td style="white-space:nowrap;color:#374151"><?= e(date('M j, H:i:s', strtotime((string) $item['ts']))) ?></td>
                            <td>
                            ...
                            <td><code style="background:#f3f4f6;padding:.1rem .4rem;border-radius:3px;font-size:11.5px"><?= e($item['event_type']) ?></code></td>
                            <td style="font-family:monospace;font-size:11.5px;color:#374151;max-width:340px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= e((string) $item['subject_label']) ?></td>
                            <td><a href="/admin/webhooks/<?= e($item['source']) ?>/<?= (int) $item['id'] ?>" style="font-size:11.5px;color:#4f46e5;text-decoration:none;font-weight:600">Inspect →</a></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <aside class="wh-sidebar">
            <div class="card" style="margin-bottom:.85rem">
                <h3>Providers</h3>
                <p style="color:#9ca3af;font-size:11px;margin:0 0 .5rem;line-height:1.4">Click to filter by provider</p>
                <div class="wh-chip-list">
                    <?php
                    $chipMeta = [
                        'email:ses'      => ['SES',       '#fef3c7', '#92400e'],
                        'email:sendgrid' => ['SendGrid',  '#dbeafe', '#1e40af'],
            ...
            <div class="card">
                ...
                <p style="color:#6b7280;font-size:11.5px;line-height:1.45;margin:0">
