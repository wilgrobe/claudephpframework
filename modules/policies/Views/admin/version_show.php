...
<div style="font-size:12px;color:var(--color-gray-500);margin-bottom:.25rem">
    <a href="/admin/policies/<?= (int) $kind['id'] ?>" style="color:var(--color-primary);text-decoration:none">
...
<div style="color:var(--color-gray-500);font-size:13px;margin-bottom:1.5rem">
...
<div class="card">
    ...
    <table class="table" style="width:100%;font-size:12.5px;margin:0">
        ...
        <tbody>
            <?php if (empty($sample)): ?>
                <tr><td colspan="4" style="padding:1rem;text-align:center;color:var(--color-gray-500)">Nobody has accepted this version yet.</td></tr>
            <?php else: foreach ($sample as $a):
                $ipText = $a['ip_address'] ? @inet_ntop($a['ip_address']) : '';
            ?>
                <tr style="border-top:1px solid var(--color-gray-100)">
                    <td style="padding:.4rem .75rem;color:var(--color-gray-500);white-space:nowrap"><?= htmlspecialchars(date('M j, Y g:ia', strtotime((string) $a['accepted_at'])), ENT_QUOTES) ?></td>
                    <td style="padding:.4rem .75rem">
                        <?php if ($a['username']): ?>
                            <a href="/admin/users/<?= (int) $a['user_id'] ?>" style="color:var(--color-primary);text-decoration:none"><?= htmlspecialchars((string) $a['username'], ENT_QUOTES) ?></a>
                        <?php else: ?>
                            <em style="color:var(--color-gray-400)">(erased user)</em>
                        <?php endif; ?>
                    </td>
                    <td style="padding:.4rem .75rem;font-family:monospace;font-size:11px;color:var(--color-gray-500)"><?= htmlspecialchars((string) ($ipText ?? ''), ENT_QUOTES) ?></td>
