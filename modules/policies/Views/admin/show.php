...
<div style="font-size:12px;color:var(--color-gray-500);margin-bottom:.25rem">
    <a href="/admin/policies" style="color:var(--color-primary);text-decoration:none">← Policies</a>
</div>
<h1 style="margin:0 0 1rem;font-size:1.3rem;font-weight:700">
    ...
    <?php if ($kind['is_system']): ?>
        <span style="font-size:11px;background:var(--color-gray-100);color:var(--color-gray-500);padding:.15rem .5rem;border-radius:999px;margin-left:.25rem">system</span>
    <?php endif; ?>
</h1>
...
<div class="card" style="margin-bottom:1rem">
    <div class="card-body" style="padding:1rem 1.25rem">
        ...
        <p style="margin:0 0 .75rem;color:var(--color-gray-500);font-size:13px;line-height:1.55">
...
<div class="card" style="margin-bottom:1rem">
    <div class="card-body" style="padding:1rem 1.25rem">
        ...
        <form method="POST" action="/admin/policies/<?= (int) $kind['id'] ?>/bump"
            ...
            <?= csrf_field() ?>
            <label>
                <span style="display:block;font-size:11.5px;color:var(--color-gray-500);margin-bottom:.15rem">Version label</span>
                <input type="text" name="version_label" required placeholder="1.0" style="width:100px">
            </label>
            <label>
                <span style="display:block;font-size:11.5px;color:var(--color-gray-500);margin-bottom:.15rem">Effective date</span>
                <input type="date" name="effective_date" value="<?= date('Y-m-d') ?>">
            </label>
            <label>
                <span style="display:block;font-size:11.5px;color:var(--color-gray-500);margin-bottom:.15rem">Summary of changes (shown to users)</span>
...
<div class="card" style="margin-bottom:1rem">
    <div class="card-body" style="padding:1rem 1.25rem;display:flex;gap:1rem;align-items:center">
        <div style="font-size:1.6rem;font-weight:700;color:var(--color-primary)">
            <?= number_format($stats['ratio'] * 100, 1) ?>%
        </div>
        <div style="font-size:13px;color:var(--color-gray-700);line-height:1.5">
    ...
    <table class="table" style="width:100%;font-size:13px;margin:0">
        ...
        <tbody>
            <?php if (empty($versions)): ?>
                <tr><td colspan="6" style="padding:1.5rem;text-align:center;color:var(--color-gray-500)">No versions yet. Bump one above.</td></tr>
            ...
            ?>
                <tr style="border-top:1px solid var(--color-gray-100);<?= $current ? 'background:#f5f3ff' : '' ?>">
                    ...
                    <td style="padding:.5rem .75rem;color:var(--color-gray-500)"><?= htmlspecialchars((string) ($v['author_username'] ?? '—'), ENT_QUOTES) ?></td>
                    <td style="padding:.5rem .75rem;color:var(--color-gray-700);font-size:12px"><?= htmlspecialchars((string) ($v['summary'] ?? ''), ENT_QUOTES) ?></td>
