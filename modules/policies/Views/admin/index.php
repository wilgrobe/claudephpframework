                ...
                <tr style="border-top:1px solid var(--color-gray-100)">
                    <td style="padding:.5rem .75rem">
                        ...
                        <?php if ($k['requires_acceptance']): ?>
                            <span style="font-size:10px;background:var(--color-info-bg);color:var(--color-info-fg);padding:.1rem .35rem;border-radius:999px;margin-left:.35rem">REQUIRED</span>
                        <?php endif; ?>
                        <?php if ($k['is_system']): ?>
                            <span style="font-size:10px;background:var(--color-gray-100);color:var(--color-gray-500);padding:.1rem .35rem;border-radius:999px;margin-left:.25rem">system</span>
                        <?php endif; ?>
                        <div style="font-size:11.5px;color:var(--color-gray-400);margin-top:.15rem">slug: <code><?= htmlspecialchars((string) $k['slug'], ENT_QUOTES) ?></code></div>
                    </td>
                    ...
                    <td style="padding:.5rem .75rem">
                        <?php if ($k['current_version_id']): ?>
                            ...
                            <div style="font-size:11.5px;color:var(--color-gray-500)">eff. <?= htmlspecialchars(date('M j, Y', strtotime((string) ($k['current_effective_date'] ?? 'now'))), ENT_QUOTES) ?></div>
                        <?php else: ?>
                            <em style="color:var(--color-gray-400)">never published</em>
                        <?php endif; ?>
                    </td>
    ...
    <div class="card-body" style="padding:1rem">
        <form method="POST" action="/admin/policies/kinds" style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;align-items:end">
            ...
            <label style="display:block">
                <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">Slug (alphanumeric)</span>
                <input type="text" name="slug" required style="width:100%" placeholder="cookie_policy">
            </label>
            <label style="display:block">
                <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">Label</span>
                <input type="text" name="label" required style="width:100%" placeholder="Cookie Policy">
            </label>
            <label style="display:block;grid-column:1 / -1">
                <span style="display:block;font-size:12px;color:var(--color-gray-500);margin-bottom:.2rem">Description</span>
