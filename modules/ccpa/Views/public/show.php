...
<p style="margin:0 0 1.5rem;color:var(--color-gray-500);font-size:14px;line-height:1.6">
...
<details style="margin:1rem 0">
    <summary style="cursor:pointer;color:var(--color-gray-500);font-size:13px">Withdraw opt-out (rare — opt back in)</summary>
    <form method="POST" action="/do-not-sell/withdraw"
        ...
        <?= csrf_field() ?>
        <?php if (!$user): ?>
            <label for="email" style="display:block;font-size:13px;color:var(--color-gray-700);margin-bottom:.4rem">Email used for the previous opt-out</label>
...
<div class="card" style="margin-bottom:1.25rem">
    <form method="POST" action="/do-not-sell">
        ...
        <div class="card-body" style="padding:1.25rem">
            ...
            <p style="margin:0 0 1rem;color:var(--color-gray-500);font-size:13px;line-height:1.55">
            ...
            <?php if (!$user): ?>
                ...
                <input type="email" name="email" required
                       style="width:100%;max-width:380px;padding:.55rem .75rem;border:1px solid var(--color-gray-300);border-radius:6px;font-size:13.5px"
                       placeholder="you@example.com" id="email">
                <div style="font-size:12px;color:var(--color-gray-500);margin-top:.35rem;line-height:1.5">
                    Required so we can match the opt-out against any future
                    records bearing your address. We won't add you to any
                    mailing list.
                </div>
            <?php else: ?>
                <p style="margin:0 0 .75rem;font-size:13px;color:var(--color-gray-700)">
...
<div style="margin-top:2rem;padding-top:1rem;border-top:1px solid var(--color-gray-200);color:var(--color-gray-500);font-size:12.5px;line-height:1.55">
