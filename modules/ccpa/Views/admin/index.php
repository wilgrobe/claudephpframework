...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
    <div>
        ...
        <p style="margin:.25rem 0 0;color:var(--color-gray-500);font-size:13.5px">
            ...
            <a href="/admin/settings/access" style="color:var(--color-primary);text-decoration:none">/admin/settings/access</a>.
        </p>
    </div>
</div>
...
<div class="card">
    ...
    <table class="table" style="width:100%;font-size:13px;margin:0">
        ...
        <tbody>
            <?php if (empty($recent)): ?>
                <tr><td colspan="4" style="padding:1.5rem;text-align:center;color:var(--color-gray-500)">No opt-outs recorded yet.</td></tr>
            <?php else: foreach ($recent as $r):
                ...
                $color = $sourceColors[$r['source']] ?? 'var(--color-gray-500)';
                $isWithdrawn = !empty($r['withdrawn_at']);
            ?>
                <tr style="border-top:1px solid var(--color-gray-100);<?= $isWithdrawn ? 'opacity:.55' : '' ?>">
                    <td style="padding:.5rem .75rem;color:var(--color-gray-500);font-size:12px;white-space:nowrap"><?= e(date('M j, g:ia', strtotime((string) $r['created_at']))) ?></td>
                    <td style="padding:.5rem .75rem">
                        <?php if ($r['user_username']): ?>
                            <a href="/admin/users/<?= (int) $r['user_id'] ?>" style="color:var(--color-primary);text-decoration:none"><?= e($r['user_username']) ?></a>
                            <span style="color:var(--color-gray-400);font-size:11px">— <?= e($r['user_email']) ?></span>
                        <?php elseif ($r['email']): ?>
                            <span style="font-family:ui-monospace,monospace;font-size:12px;color:var(--color-gray-700)"><?= e($r['email']) ?></span>
                            <span style="color:var(--color-gray-400);font-size:11px;margin-left:.25rem">(guest)</span>
                        <?php else: ?>
                            <em style="color:var(--color-gray-400);font-size:12px">cookie-only opt-out</em>
                        <?php endif; ?>
                    </td>
                    <td style="padding:.5rem .75rem">
                    ...
                    <td style="padding:.5rem .75rem;color:var(--color-gray-500);font-size:12px">
                        ...
                        <?php else: ?>
                            <span style="color:var(--color-success);font-weight:500">active</span>
                        <?php endif; ?>
                    </td>
                </tr>
